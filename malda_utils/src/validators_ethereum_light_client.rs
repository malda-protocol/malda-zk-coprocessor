...
use crate::constants::*;
use crate::types::*;
use crate::zkvm_input_helper_ethereum_light_client::*;
...
impl L1ChainBuilder {
    ...
    pub fn new() -> Self {
        ...
        forks.electra.fork_version = fixed_bytes!("05000000");
        let genesis_root =
...
pub fn validate_get_proof_data_call(
    ...
    let env = env_input.into_env(&ETH_MAINNET_CHAIN_SPEC);
    ...
    let proof_data = contract.call_builder(&call).call();
    ...
    let env = beacon_input.into_env(&ETH_MAINNET_CHAIN_SPEC);
...
pub fn validate_ethereum_env_via_sync_committee(
    ...
    let new_checkpoint = l1_chain_builder
        .last_checkpoint
        .map_or_else(|| checkpoint, |last| last);

...
mod tests {
  ...
  async fn test_validate_linea_env_correct_input() {
    ...
    // Verify the header
    let result = linea_block_verifier::core::verify_header(untrusted_header, &beacon_data, network);
    assert!(result.is_ok());
  }
  ...
  async fn test_validate_linea_env_input_manipulated_panics() {
    ...
    let result =
      linea_block_verifier::core::verify_header(&manipulated_header, &beacon_data, network);
    assert!(result.is_err());
  }

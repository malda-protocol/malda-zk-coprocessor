  ...
  async fn test_get_proof_data_with_params(
    ...
    for i in 0..expected_length {
      ...
      assert_eq!(
        dst_chain_ids[i].len(),
        user_count,
        "Dst chain IDs inner vector length should match users inner vector length at index {}",
        i
      );
    }
    ...
    if boundless {
      ...
      let journals: Vec<Bytes> =
        <Vec<Bytes>>::abi_decode(&session_info.journal.bytes).expect("Failed to decode journal");

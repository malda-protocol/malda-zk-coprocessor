...
use crate::constants::*;
use crate::prover::{prove_bonsai_from_env, prove_boundless, BonsaiProveInfo, SessionStats};
#[cfg(feature = "guest")]
...
#[cfg(feature = "guest")]
use crate::prover::{execute, prove_local};
#[cfg(feature = "guest")]
use risc0_zkvm::{ExecutorEnv, ProveInfo, SessionInfo};
...
// Re-export prover types for backward compatibility
pub use crate::prover::BoundlessParams;

/// Type alias for backward compatibility with existing code.
/// Use `SessionStats` from `crate::prover` for new code.
pub type MaldaSessionStats = SessionStats;

/// Type alias for backward compatibility with existing code.
/// Use `BonsaiProveInfo` from `crate::prover` for new code.
pub type MaldaProveInfo = BonsaiProveInfo;
...
pub async fn get_proof_data_prove_boundless_with_elf(
    ...
    // Use the generic prover module to submit to Boundless
    prove_boundless(elf, input_bytes, boundless_params, onchain).await
}
...
pub async fn get_proof_data_exec(
    ...
    Ok(execute(env, GET_PROOF_DATA_ELF).expect("Failed to execute ZKVM"))
}
...
pub async fn get_proof_data_prove(
    ...
    let prove_info = tokio::task::spawn_blocking(move || {
        ...
        let start_time = std::time::Instant::now();
        let proof = prove_local(env, GET_PROOF_DATA_ELF);
        let duration = start_time.elapsed();
        info!("Local proof time: {:?}", duration);
        proof
    })
    .await?;
...
pub async fn get_proof_data_prove_sdk(
    ...
    let prove_info = tokio::task::spawn_blocking(move || {
        ...
        let start_time = std::time::Instant::now();
        let proof = prove_bonsai_from_env(input);
        let duration = start_time.elapsed();
